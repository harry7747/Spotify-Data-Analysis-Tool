...
def display_most_streamed_artist(data, sp):
    ...
    results = sp.search(q=most_streamed_artist, type='artist', limit=1)
    artist_image_url = results["artists"]["items"][0]["images"][0]["url"] if results["artists"]["items"] else None 
...
def display_top_5_artists(data):
    top_5_artists = top_streamed_artists()
    
    display_html = "<h3>🎤 Top 5 Most Streamed Artists</h3><ul>"
    
    for artist, count in top_5_artists.items():
        display_html += f"<li><strong>{artist}</strong>: {count} streams</li>"
...
def visualize_yearly_listening_time(data):
    data["year"] = pd.to_datetime(data["endTime"]).dt.year
...
def visualize_most_streamed_day(data):
...
def visualize_top_5_listening_days(data):
...
def display_top_5_listening_days(data):
    ...
    top_5_days["hours_played"] = (top_5_days["ms_played"] / (1000 * 60 * 60)).round(1)

    display_html = "<h3>🎧 Your Top 5 Listening Days</h3><ul>"
    ...
    return display(HTML(display_html))
...
def visualize_listening_diversity(data, time_period="year", show_graph=False):
    """
